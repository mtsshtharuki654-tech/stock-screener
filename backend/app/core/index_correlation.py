...
def get_correlation_for_stock(
    stock_daily: pd.DataFrame,
    segment: str,
    index_cache: "dict[str, pd.DataFrame] | None" = None,
) -> IndexCorrelation | None:
    ...
    # 事前ロード済みキャッシュがあればそれを使う（ネットワーク不使用）
    if index_cache is not None:
        ...
        try:
            ...
            cov_mat = np.cov(s, m)
            beta = float(cov_mat[0, 1] / cov_mat[1, 1]) if cov_mat[1, 1] != 0 else 0.0
            label = "指数連動型" if corr >= 0.6 else "逆相関型" if corr <= -0.3 else "独自動き型"
            beta_label = None
            if beta > 1.5:
                beta_label = f"高ベータ（指数の{beta:.1f}倍動く）"
            elif beta < 0.5:
                beta_label = "低ベータ（指数の動きに鈍感）"
            return IndexCorrelation(
                index_name=index_name, beta=round(beta, 2),
                correlation=round(corr, 2), label=label, beta_label=beta_label,
            )
        except Exception:
            return None
